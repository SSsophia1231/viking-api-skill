...
collection_name = "your collection" #知识库名称
...
account_id = "your account_id" #火山账号
g_knowledge_base_domain = "api-knowledgebase.mlp.cn-beijing.volces.com"
model_name = "Doubao-vision-pro-32k" #使用的模型名
...
"""
#准备请求：通过构造 HTTP 请求并生成签名，确保请求的合法性和安全性
...
#知识检索：向知识库服务发送查询请求，获取与用户问题相关的参考资料
...
#对话补全：调用多模态大模型 API，生成基于用户问题和参考资料的回答
...
def get_content_for_prompt(point: dict, image_num: int) -> str:
    ...
    if image_num > 0 and point.get("chunk_attachment") and point["chunk_attachment"][0].get("link"):
        placeholder = f"<img>图片{image_num}</img>"
        return content + placeholder
    return content
    
#生成 Prompt：根据知识检索结果动态生成适配的文本或视觉问答模板
def generate_prompt(rsp_txt):
    ...
    if using_vlm:
        return vision_base_prompt.format(documents=prompt), image_urls
    return base_prompt.format(documents=prompt), image_urls

#主流程控制：将知识检索、Prompt生成、模型回答串联为完整的问答流程，支持图文结合
