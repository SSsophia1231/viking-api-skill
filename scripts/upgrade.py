...
- Download changed files, delete removed files, update local meta/version
"""
...
GITHUB_RAW_BASE = "https://raw.githubusercontent.com/SSsophia1231/viking-api-skill/main/resources"
...
def _remote_file_url(relative_path: str) -> str:
    return f"{GITHUB_RAW_BASE}/{quote(relative_path, safe='/')}"
...
def main() -> int:
    ...
    try:
        ...
        print("proceeding with local resources")
        return 0
    ...
    try:
        ...
        print(f"warning: could not fetch meta.json: {exc}", file=sys.stderr)
        return 0

    if not isinstance(remote_meta.get("files"), list):
        print("warning: invalid remote meta format, skipping update", file=sys.stderr)
        return 0
